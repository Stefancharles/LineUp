...
static  void enc28j60_exit_configuration(void)
{
    ...
    EXTI_InitStructure.EXTI_Trigger = EXTI_Trigger_Falling ; /*é?éy??′￥·￠*/
    EXTI_InitStructure.EXTI_LineCmd = ENABLE;
    EXTI_Init(&EXTI_InitStructure);
    EXTI_GenerateSWInterrupt(ENC28J60_INT_EXTI_LINE);	/*?D??1òμ? EXTI_Line0  ??*/
}
...
/**
  * @brief  功能说明: 读缓冲区
	...
	*      		使用方法: 替代read data into( q->payload, q->len );
  * @param  unsigned int len        读取长度
...
/**
  * @brief  设定寄存器存储区域
  * @param  unsigned char address   寄存器地址
  * @retval None
  */
...
/**
  * @brief  功能说明: 写物理寄存器
	...
	*         					详见数据手册19页
  * @param  unsigned char address   物理寄存器地址
...
/**
  * @brief  初始化enc28j60
  * @param  unsigned char* mac_addr  mac地址参数指针
  * @retval None
  */
...
/**
  * @brief  功能说明: 数据包发送函数【1】 修改发送缓冲区，并发送控制字节.
	*         使用说明: 控制字节为0，采用macon3参数，自动填充0，使能CRC校验
											适用于LwIP，在low_level_out中使用，替代initiate transfer();
...
/**
  * @brief  功能说明: 通过enc28j60接收数据包
						使用说明: 仅适用于uIP
  * @param  unsigned int maxlen       接收数据包最大长度值
...
/**
  * @brief  功能说明: 结束读取接收缓冲区过程，移动接收缓冲区指针
